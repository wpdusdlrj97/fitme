    ...
    <nav class="navbar-default navbar-side" role="navigation">
        <div class="sidebar-collapse">
            <ul class="nav" id="main-menu">
                ...
                <li>
                    <a href="product_approval.php"><i class="fa fa-qrcode "></i>제품관리</a>
                </li>
                <li>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">색상 추가</h4>
            </div>
            <div class="modal-body">
                <div class="colors_button">
                    <div class="choose_color_label">색상추가</div>
                </div>
                <div class="choose_color_box">
                </div>
                <div class="add_color_line"></div>
                <div class="add_color_button">추가</div>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    var list_image;
    var color_name_array = new Array();
    var color_rgb_array = new Array();//색상
    var color_object = new Object();
    ...
    $('.add_color_button').click(function(){
        if($('.choose_color').length>0)
        {
            ...
            for(var i=0;i<$('.choose_color').length;i++)
            {
                ...
                color_name_array.push($($('.choose_color').eq(i)).children('.color_name_value').val());
                color_rgb_array.push($($('.pickr').eq(i)).children('button').css("color"));
                var add_child = "<div class=\"color_child_box\" style=\"border:2px #E0E6F8 solid; height:40px; border-radius:5px; margin:20px 20px 0 0; float:left; line-height:40px; background-color:#EFF2FB\">";
                add_child+="<div class=\"color_child_box_background_color\" style=\"width:20px; height:20px; border-radius:2px; background-color:"+$($('.pickr').eq(i)).children('button').css("color")+"; float:left; margin:9px 0 0 10px;\"></div>";
                add_child+="<div class=\"color_child_box_name\" style=\"height:40px; float:left; line-height:39px; font-size:14px; font-family: 'Noto Sans KR', sans-serif; margin:0 0 0 10px;\">"+$($('.choose_color').eq(i)).children('.color_name_value').val()+"</div>";
                add_child+="<div class=\"color_child_box_remove\" style=\"height:40px; float:left; line-height:41px; color:#A9BCF5; font-size:13px; cursor:pointer; font-family: 'Noto Sans KR', sans-serif; margin:0 10px 0 15px;\">X</div></div>";
                $('.color_box').append(add_child);
            }
            $('.color_child_box_remove').hover(function() {
                $(this).css("color", "#819FF7");
            }, function(){
                $(this).css("color", "#A9BCF5");
            });
            $('.color_child_box_remove').click(function(){
                var index = $($(this).parent('div')).index();
                $($(this).parent('div')).remove();
                $($('.choose_color_delete').eq(index).parent("div")).remove();
                color_name_array.splice(index,1);
                color_rgb_array.splice(index,1);
                if($('.color_child_box').length<1)
                {
    ...
    //색상추가모달창 내부에서 색상 변경시
    var color_index=-1;
    $('.choose_color_label').click(function(){
        color_index++;
        var tag = "<div class=\"choose_color\" style=\"width:100%; height:60px; margin-bottom:10px;\">";
        tag+="<label class=\"color_name\" for=\"color_name_value\" style=\"float:left; text-align:center; width:60px; height:40px; margin-top:15px; line-height:40px;\">색상이름</label>";
        tag+="<input id=\"color_name_value\" type=\"text\" placeholder=\"색상명 입력\" class=\"color_name_value\" style=\"padding-left:5px; border:1px solid; border-radius:2px; float:left; width:130px; margin-left:15px; height:40px; margin-top:15px; line-height:40px;\">";
        tag+="<div class=\"choose_back_parent\" style=\"width:40px; height:35px; margin-left:15px; margin-top:20px; float:left;\"><div class=\"choose_color_background\"></div></div>";
        tag+="<div class=\"choose_color_delete\" style=\"width:40px; border:1px solid; height:40px; margin-left:10px; margin-top:15px; float:left; font-size:18px; line-height:40px; text-align: center; background-color: #424242; border-radius:5px; cursor:pointer; color:white;\">X</div></div>";
        $('.choose_color_box').append(tag);
        var pickr = Pickr.create({
            el: '.choose_color_background',
        ...
        $('.choose_color_delete').click(function(){
            $($(this).parent("div")).remove();
        });
    });
    ...
    */
    ...
    function product_upload_top_post()
    {
        ...
        else if($('.color_child_box').length<1)
        {
            swal({
                title: "제품 등록",
                text: "색상을 하나 이상 추가하세요",
                icon: "warning",
                dangerMode: true
            });
        }
    ...
    function setForm(editor) {
        swal({
            ...
            if (willDelete) {
                $('#product_upload_body').append("<div class=\"loader loader-default is-active\" style=\"float:left;\" data-text=\"잠시 기다려주세요\" data-blink ></div>");
                var images = new FormData();
                color_object['name']=color_name_array;
                color_object['rgb']=color_rgb_array;
                var color_send_data = JSON.stringify(color_object);

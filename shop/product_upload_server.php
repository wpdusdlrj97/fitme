...
if($email)
{
    ...
    if($row['level']==1)
    {
        ...
        if($number)
        {
            ...
            {
              $qry = mysqli_query($connect,"update product set name='$name',ex='$ex',price=$price,category1='$category1',category2='$category2',size='$size',content='$content',stock=$stock,color='$color' where product_key=$key");
            }
        }
        //넘겨받은 데이터는 파일 데이터
        else
        {
            if(count($_FILES['images']['name'])>2)
            {
                ...
                $qry = mysqli_query($connect,"insert into product(email,name,ex,price,category1,category2,size,content, main_image, detail_image,stock,date,color,likes,click,review_count) values('$email','-','-',0,'-','-','-','-','$main_url','$json',0,$date,'-',0,0,0);");
                $row = mysqli_fetch_array($qry);

            ...
            <table id="table"> <!-- 제품 리스트가 있는 테이블이다. -->
                ...
                <tr style="height:180px;">
                    ...
                    ?> ); background-size:100% 100%;" product-key="<?php echo $product;?>"
                        product-url="<?php echo $row['url'];?>" product-name="<?php echo $row['name'];?>" product-size="<?php echo str_replace("\"","@",$row['size']); ?>"
                        product-image-size="<?php $size_temp_array = getimagesize($row['url']);  echo $size_temp_array[0].'@'.$size_temp_array[1] ?>"
...
<script>
    ...
    */
    ...
    function image_move(object)
    {
        ...
        var clothes_w = JsonLine["x_finish"]-JsonLine["x_start"]; //원본 이미지 내부의 옷의 width
        var clothes_y =  JsonLine["y_finish"]-JsonLine["y_start"]; //원본 이미지 내부의 옷의 height
